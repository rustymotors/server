...
export class SocketMgrCompletion extends SubThread {
    connectedClients: Connection[];
    connectionID: number;
    loginServer: Server | null = null;
    personaServer: Server | null = null;
    lobbyServer: Server | null = null;
    dbServer: Server | null = null;
    constructor() {
    ...
    init() {
        ...
        console.log(
            `SocketMgrCompletion listening on port ${MC_DB_CLIENT_PORT}`
        );

        this.personaServer = createServer(this.onSocketConnection.bind(this));
        this.personaServer.listen(
            MC_PERSONA_PORT,
            "0.0.0.0",
            INITIAL_CONNECTIONS
        );
    ...
    onSocketConnection(socket: Socket) {
        console.log(
            `SocketMgrCompletion.onConnection() socket.remoteAddress: ${socket.remoteAddress}, socket.remotePort: ${socket.remotePort}`
        );
        const con = new Connection();
        con.id = this.connectionID++;
        if (typeof socket.remoteAddress != "string") {
            console.log(
                `SocketMgrCompletion.onConnection() socket.remoteAddress is not a string.`
            );
            return;
        }
        con.ip = socket.remoteAddress;
        console.log(
            `SocketMgrCompletion.onConnection() con.id: ${con.id}, con.ip: ${con.ip}`
        );
        ...
        if (typeof socket.localPort != "number") {
            console.log(
                `SocketMgrCompletion.onConnection() socket.localPort is not a number.`
            );
            return;
        }
