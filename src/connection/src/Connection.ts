...
export class Connection implements IConnection{
  ...
  private async _getCiperKeyFromDatabase() {
    ...
    const session_key = await getDatabase()
      ...
      .then((rows) => {
        if (rows.length < 1 || typeof rows[0] === "undefined") {
          this._logger.error(
            `Error getting cipher key from database for persona ID ${this.getPersonaId().toString()}`
          );
          throw new Error(
            `Error getting cipher key from database for persona ID ${this.getPersonaId().toString()}`
          );
    ...
    this._logger.debug(
      `Got cipher key from database for persona ID ${this._personaId.toString()}`
    );
  ...
  handleServerSocketData(data: Buffer): void {
    ...
    try {
      ...
      this._logger.error(
        `Error handling socket data for connectionId ${this._connectionId}: ${
  ...
  processServerMessage(message: ServerMessage) {
    ...
    if (message.isEncrypted() && this._cipherPair === null) {
      this._getCiperKeyFromDatabase().catch((error: unknown) => {
        this._logger.error(
          `Error getting cipher key from database for persona ID ${
            this._personaId?.toString() ?? ""
          }: ${error as string}`
        );
        Sentry.captureException(error);
      });
    ...
    processor(
      ...
      this._logger.error(
        `Error processing message for connectionId ${this._connectionId}: ${
          error as string
        }`
      );
      Sentry.captureException(error);
  ...
  sendServerMessage(messages: ServerMessage[]) {
    ...
    try {
      ...
      this._logger.error(
        `Error sending server message for connectionId ${this._connectionId}: ${
          error as string
        }`
      );
      Sentry.captureException(error);
