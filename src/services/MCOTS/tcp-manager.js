...
import { ClientConnectMsg } from '../MCServer/client-connect-msg.js'
import { GenericReplyMsg } from '../MCServer/generic-reply-msg.js'
import { GenericRequestMsg } from '../MCServer/generic-request-msg.js'
import { DatabaseManager } from '../shared/database-manager.js'
import { StockCar } from './stock-car.js'
import { StockCarInfoMsg } from './stock-car-info-msg.js'
import { MCOTServer } from './mcots-server.js'
 ...
 */
async function socketWriteIfOpen(connection, nodes) {
 ...
 */
async function getStockCarInfo(connection, node) {
  const getStockCarInfoMessage = new GenericRequestMsg()
  getStockCarInfoMessage.deserialize(node.data)
  getStockCarInfoMessage.dumpPacket()

  const stockCarInfoMessage = new StockCarInfoMsg(200, 0, 105)
 ...
 * @returns {Promise<{con: ConnectionObj, nodes: MessageNode[]}>}
 */
async function clientConnect(connection, node) {
  /**
   * Let's turn it into a ClientConnectMsg
   */
  // Not currently using this
  const newMessage = new ClientConnectMsg(node.data)
  ...
  const genericReplyMessage = new GenericReplyMsg()
