...
const client_connect_msg_1 = require("../MCServer/client-connect-msg");
const generic_reply_msg_1 = require("../MCServer/generic-reply-msg");
const generic_request_msg_1 = require("../MCServer/generic-request-msg");
const database_manager_1 = require("../shared/database-manager");
const stock_car_1 = require("./stock-car");
const stock_car_info_msg_1 = require("./stock-car-info-msg");
const _1 = require(".");
const message_node_1 = require("./message-node");
const { log } = mco_logger_1.Logger.getInstance();
/**
 * Manages TCP connection packet processing
 * @module TCPManager
 */
const mcotServer = new _1.MCOTServer();
const databaseManager = database_manager_1.DatabaseManager.getInstance();
/**
 *
...
async function socketWriteIfOpen(connection, packetList) {
    ...
    for (const packet of updatedConnection.packetList) {
        ...
        const compressedPacket = (await compressIfNeeded(connection, packet)).packet;
        // Does the packet need to be encrypted?
        const encryptedPacket = (await encryptIfNeeded(connection, compressedPacket)).packet;
        // Log that we are trying to write
        log('debug', ` Atempting to write seq: ${encryptedPacket.seq} to conn: ${updatedConnection.connection.id}`, { service: 'mcoserver:MCOTSServer' });
        // Log the buffer we are writing
        log('debug', `Writting buffer: ${encryptedPacket.serialize().toString('hex')}`, {
            service: 'mcoserver:MCOTSServer',
        });
        if (connection.sock.writable) {
            ...
            throw new Error(`Error writing ${encryptedPacket.serialize()} to ${connection.sock.remoteAddress} , ${connection.sock.localPort.toString()}`);
        }
    }
...
async function getStockCarInfo(connection, packet) {
    const getStockCarInfoMessage = new generic_request_msg_1.GenericRequestMessage();
    getStockCarInfoMessage.deserialize(packet.data);
    getStockCarInfoMessage.dumpPacket();
    const stockCarInfoMessage = new stock_car_info_msg_1.StockCarInfoMessage(200, 0, 105);
    stockCarInfoMessage.starterCash = 200;
    stockCarInfoMessage.dealerId = 8;
    stockCarInfoMessage.brand = 105;
    stockCarInfoMessage.addStockCar(new stock_car_1.StockCar(113, 20, 0)); // Bel-air
    stockCarInfoMessage.addStockCar(new stock_car_1.StockCar(104, 15, 1)); // Fairlane - Deal of the day
    stockCarInfoMessage.addStockCar(new stock_car_1.StockCar(402, 20, 0)); // Century
    stockCarInfoMessage.dumpPacket();
    const responsePacket = new message_node_1.MessageNode(message_node_1.EMessageDirection.SENT);
...
async function clientConnect(connection, packet) {
    ...
    const newMessage = new client_connect_msg_1.ClientConnectMessage(packet.data);
    log('debug', `[TCPManager] Looking up the session key for ${newMessage.customerId}...`, { service: 'mcoserver:MCOTSServer' });
    const result = await databaseManager.fetchSessionKeyByCustomerId(newMessage.customerId);
    ...
    const genericReplyMessage = new generic_reply_msg_1.GenericReplyMessage();
...
async function messageReceived(message, con) {
    ...
    if (message.flags !== 80 && newConnection.useEncryption) {
        if (!newConnection.isSetupComplete) {
            throw new Error(`Decrypt() not yet setup! Disconnecting...conId: ${con.id}`);
        }
        if (message.flags - 8 >= 0) {
            try {
                ...
                log('debug', `Full packet before decrypting: ${encryptedBuffer.toString('hex')}`, { service: 'mcoserver:MCOTSServer' });
                log('debug', `Message buffer before decrypting: ${encryptedBuffer.toString('hex')}`, { service: 'mcoserver:MCOTSServer' });
                ...
                if (error instanceof Error) {
                    throw new TypeError(`Decrypt() exception thrown! Disconnecting...conId:${newConnection.id}: ${error}`);
                }
                throw new Error(`Decrypt() exception thrown! Disconnecting...conId:${newConnection.id}, error unknown`);
            }
        }
    }
...
async function defaultHandler(rawPacket) {
    ...
    const messageNode = new message_node_1.MessageNode(message_node_1.EMessageDirection.RECEIVED);
