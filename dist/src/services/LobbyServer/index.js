...
const nps_user_info_1 = require("./nps-user-info");
...
async function npsSocketWriteIfOpen(conn, buffer) {
    ...
    if (sock.writable) {
        ...
        throw new Error(`[Lobby] Error writing ${buffer.toString('hex')} to ${sock.remoteAddress} , ${sock}`);
    }
    return conn;
}
/**
...
async function sendCommand(con, data) {
    const s = con;
    const decipheredCommand = decryptCmd(s, Buffer.from(data.slice(4))).decryptedCmd;
    // Marshal the command into an NPS packet
    const incommingRequest = new nps_msg_1.NPSMessage(message_node_1.EMessageDirection.RECEIVED);
    ...
    // Build the packet
    const packetResult = new nps_msg_1.NPSMessage(message_node_1.EMessageDirection.SENT);
...
class LobbyServer {
    ...
    async dataHandler(rawPacket) {
        ...
        log('debug', `Received Lobby packet: ${JSON.stringify({ localPort, remoteAddress })}`, { service: 'mcoserver:LobbyServer' });
        ...
        switch (requestCode) {
            ...
            case '100': {
                const responsePacket = await this._npsRequestGameConnectServer(connection, data);
                log('debug', `Connect responsePacket's data prior to sending: ${JSON.stringify({
            ...
            case '217': {
                ...
                log('debug', `Heartbeat responsePacket's data prior to sending: ${JSON.stringify({
                    data: responsePacket.getPacketAsString(),
                })}`, { service: 'mcoserver:LobbyServer' });
                npsSocketWriteIfOpen(connection, responsePacket.serialize());
                break;
            ...
            case '1101': {
                ...
                if (encryptedCmd === null) {
                    throw new Error(`Error with encrypted command, dumping connection: ${JSON.stringify({ updatedConnection })}`);
                }
                log('debug', `encrypedCommand's data prior to sending: ${JSON.stringify({
            ...
            default:
                throw new Error(`[Lobby] Unknown code ${requestCode} was received on port 7003`);
        }
    ...
    async _npsRequestGameConnectServer(connection, rawData) {
        ...
        log('debug', `_npsRequestGameConnectServer: ${JSON.stringify({
            remoteAddress: sock.remoteAddress,
            data: rawData.toString('hex'),
        })}`, { service: 'mcoserver:LobbyServer' });
        // Return a _NPS_UserInfo structure
        const userInfo = new nps_user_info_1.NPSUserInfo(message_node_1.EMessageDirection.RECEIVED);
        userInfo.deserialize(rawData);
        userInfo.dumpInfo();
        const personaManager = persona_server_1.PersonaServer.getInstance();
        const personas = await personaManager.getPersonasByPersonaId(userInfo.userId);
        ...
        const keys = await databaseManager
            ...
            .catch(error => {
            log('error', `Unable to fetch session key for customerId ${customerId}: ${error}`, {
                service: 'mcoserver:LobbyServer',
            });
            return undefined;
        });
        ...
        if (!s.encLobby.decipher) {
            try {
                ...
                if (error instanceof Error) {
                    throw new TypeError(`Unable to set session key: ${JSON.stringify({ keys, error })}`);
                }
                throw new Error(`Unable to set session key: ${JSON.stringify({
                    keys,
                    error: 'unknown',
                })}`);
            }
