...
const nps_persona_maps_msg_1 = require("./nps-persona-maps-msg");
...
class PersonaServer {
    ...
    async handleSelectGamePersona(data) {
        ...
        const requestPacket = new nps_msg_1.NPSMessage(message_node_1.EMessageDirection.RECEIVED).deserialize(data);
        log('debug', `NPSMsg request object from _npsSelectGamePersona: ${JSON.stringify({
        ...
        // Response Code
        // 207 = success
        const responsePacket = new nps_msg_1.NPSMessage(message_node_1.EMessageDirection.SENT);
      ...
      ${JSON.stringify({
        ...
        log('debug', `[npsSelectGamePersona] responsePacket's data prior to sending: ${responsePacket.getPacketAsString()}`, { service: this.serviceName });
    ...
    async createNewGameAccount(data) {
      ...
      ${JSON.stringify({
        ...
        requestPacket.dumpPacket();
        const rPacket = new nps_msg_1.NPSMessage(message_node_1.EMessageDirection.SENT);
    ...
    async logoutGameUser(data) {
      ...
      ${JSON.stringify({
        ...
        responsePacket.dumpPacket();
        log('debug', `[npsLogoutGameUser] responsePacket's data prior to sending: ${responsePacket.getPacketAsString()}`, { service: this.serviceName });
        return responsePacket;
    }
    /**
    ...
    async validateLicencePlate(data) {
      ...
      ${JSON.stringify({
        ...
        responsePacket.dumpPacket();
        log('debug', `[npsCheckToken] responsePacket's data prior to sending: ${responsePacket.getPacketAsString()}`, { service: this.serviceName });
        return responsePacket;
    }
    /**
    ...
    async validatePersonaName(data) {
      ...
      ${JSON.stringify({
        ...
        log('debug', JSON.stringify({ customerId, requestedPersonaName, serviceName }), {
            service: this.serviceName,
        });
        // Create the packet content
      ...
      ${JSON.stringify({
        ...
        responsePacket.dumpPacket();
        log('debug', `[npsValidatePersonaName] responsePacket's data prior to sending: ${responsePacket.getPacketAsString()}`, { service: this.serviceName });
        return responsePacket;
    }
    /**
     *
    ...
    async getPersonasByCustomerId(customerId) {
        ...
        if (results.length === 0) {
            return Promise.reject(new Error(`Unable to locate a persona for customerId: ${customerId}`));
        }
        return results;
    }
    /**
    ...
    async getPersonaMaps(data) {
      ...
      ${JSON.stringify({
        ...
        const personas = await this.getPersonaMapsByCustomerId(customerId.readUInt32BE(0));
        log('debug', `${personas.length} personas found for ${customerId.readUInt32BE(0)}`, { service: this.serviceName });
        let responsePacket;
        const personaMapsMessage = new nps_persona_maps_msg_1.NPSPersonaMapsMessage(message_node_1.EMessageDirection.SENT);
        if (personas.length === 0) {
            throw new Error(`No personas found for customer Id: ${customerId.readUInt32BE(0)}`);
        }
        else {
            try {
                personaMapsMessage.loadMaps(personas);
                responsePacket = new nps_msg_1.NPSMessage(message_node_1.EMessageDirection.SENT);
                responsePacket.msgNo = 0x6_07;
                responsePacket.setContent(personaMapsMessage.serialize());
                log('debug', `NPSMsg response object from _npsGetPersonaMaps: ${JSON.stringify({
                    NPSMsg: responsePacket.toJSON(),
                })}`, { service: this.serviceName });
                responsePacket.dumpPacket();
            }
      ...
      ${JSON.stringify({
        ...
        switch (requestCode) {
            ...
            case '534':
                ...
                return updatedConnection;
            default:
                throw new Error(`[personaServer] Unknown code was received ${JSON.stringify({
                    requestCode,
                    localPort,
                })}`);
        }
...
//# sourceMappingURL=persona-server.js.map