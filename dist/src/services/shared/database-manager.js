...
class DatabaseManager {
    ...
    constructor() {
        this.localDB = new (sqlite3_1.default.verbose().Database)('db/mco.db');
        const { localDB } = this;
        this.changes = 0;
    ...
    async _updateSessionKey(customerId, sessionkey, contextId, connectionId) {
        ...
        return new Promise((resolve, reject) => {
            try {
                const stmt = this.localDB.prepare('REPLACE INTO sessions (customer_id, sessionkey, skey, context_id, connection_id) VALUES ($customerId, $sessionkey, $skey, $contextId, $connectionId)');
                stmt.run({
