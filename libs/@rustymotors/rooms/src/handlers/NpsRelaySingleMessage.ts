 ...
 *   LOGGED variant we'll get "Unknown command" errors. Same handler
...
export class NpsRelaySingleMessage {
    ...
    get opcode(): number { return this._opcode; }
    get length(): number { return this._length; }
    get commId(): number { return this._commId; }
    get senderUserId(): number { return this._senderUserId; }
    get filterUserId(): number { return this._filterUserId; }
    get blob(): Buffer { return Buffer.from(this._blob); }
 ...
 * Parses the envelope, emits a structured debug log, and never produces
 * responses — correct for single-player testing where there are no other
 * channel members to relay to.
 *
...
export function logRelayStub(args: {
    ...
    try {
        ...
        log.debug(`${opcodeName} received (stub — no relay)`, {
            ...
            applicationPacketName: describeApplicationPacketType(relay.applicationPacketType),
        });
    } catch (err) {
