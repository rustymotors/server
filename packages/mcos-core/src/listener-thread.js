...
export class ListenerThread {
   ...
   *
   * @static
   * @return {ListenerThread}  {ListenerThread}
   * @memberof ListenerThread
   */
  static getInstance () {
  ...
  async onTCPData (data, connection) {
    ...
    log.trace(`[listen] Raw bytes in onTCPData(pre-process): ${toHex(rawPacket.data)}`)

    /** @type {{err: Error | null, data: import("mcos-core").TCPConnection | null}} */
    const processedPacket = await processPacket(rawPacket)
    log.trace(`[listen] In onTCPData(post-process): ${errorMessage(processedPacket.err)}`)
    if (processedPacket.err || processedPacket.data === null) {
      log.error(`Error!!!: ${errorMessage(processedPacket.err)}`)
      throw new Error(
        `[listen]There was an error processing the packet: ${errorMessage(
          processedPacket.err
        )}`
      )
    }
  ...
  tcpListener (socket) {
    ...
    if (socket.localPort === 7003 && connectionRecord.modern.inQueue) {
      ...
      log.trace('[listen] In tcpListener(pre-queue)')
      socket.write(Buffer.from([0x02, 0x30, 0x00, 0x00]))
      log.trace('[listen] In tcpListener(post-queue)')
    ...
    socket.on('data', async (/** @type {Buffer} */ data) => {
      log.trace(`Logging raw bytes: ${toHex(data)}`)
      await this.onTCPData(data, connectionRecord.legacy)
      log.trace('[listen] onTCPData complete, data processed.')
    })
