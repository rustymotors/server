...
export class DatabaseManager {
            ...
            (
              ...
              "raceCashFactor" real NOT NULL
                ...
                if (err instanceof Error) {
                    const newError = new Error(`There was an error setting up the database: ${err.message}`);
    ...
    async fetchSessionKeyByCustomerId(customerId) {
        ...
        const record = await this.localDB.query('SELECT sessionkey, skey FROM sessions WHERE customer_id = $1', [customerId]);
        if (record === undefined) {
    ...
    async fetchSessionKeyByConnectionId(connectionId) {
        ...
        const record = await this.localDB.query('SELECT sessionkey, skey FROM sessions WHERE connection_id = $1', [connectionId]);
        if (record === undefined) {
            throw new Error('Unable to fetch session key');
        }
        return record.rows[0];
    ...
    async updateSessionKey(customerId, sessionkey, contextId, connectionId) {
        ...
        const record = await this.localDB.query(`INSERT INTO sessions (customer_id, sessionkey, skey, context_id, connection_id)
        VALUES ($1, $2, $3, $4, $5)
      ON CONFLICT (customer_id)
        DO UPDATE SET sessionkey = $2, skey = $3;`, [customerId, sessionkey, skey, contextId, connectionId]);
        if (record === undefined) {
            throw new Error('Unable to fetch session key');
        }
        return 1;
