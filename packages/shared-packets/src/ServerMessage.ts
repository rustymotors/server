	...
	decrypt(cipherPair: McosEncryptionPair): ServerMessage {
		...
		if (this.isEncrypted()) {
			try {
				...
				log.debug(
					`Decrypted ServerMessage with message id ${this._preDecryptedMessageId}, new message id: ${this.data.getMessageId()}`,
				);
	...
	encrypt(cipherPair: McosEncryptionPair): ServerMessage {
		...
		if (!this.isEncrypted()) {
			try {
				...
				log.debug(
					`Encrypted ServerMessage with message id ${this._preEncryptedMessageId}, new message id: ${this.data.getMessageId()}`,
				);
