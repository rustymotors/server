...
export class TCPConnection {
    ...
    setEncryptionKeyDES(skey) {
        ...
        try {
            this.encLobby.cipher = createCipheriv('des-cbc', Buffer.from(skey, 'hex'), desIV);
        ...
        try {
            this.encLobby.decipher = createDecipheriv('des-cbc', Buffer.from(skey, 'hex'), desIV);
    ...
    tryWritePackets(packetList) {
        ...
        for (const packet of updatedConnection.packetList) {
            ...
            log.debug(` Atempting to write seq: ${encryptedPacket.seq} to conn: ${updatedConnection.connection.id}`);
            ...
            if (this.sock.writable) {
                ...
                throw new Error(`Error writing ${encryptedPacket.serialize()} to ${this.sock.remoteAddress} , ${port}`);
            }
        }
