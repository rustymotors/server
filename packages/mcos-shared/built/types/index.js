        ...
        try {
            ...
            if (err instanceof Error) {
                if (err.name.includes('RangeError')) {
                    ...
                    throw new Error(`[MessageNode] Not long enough to deserialize, only ${packet.length.toString()} bytes long`);
                }
                else {
                    throw new Error(`[MessageNode] Unable to read msgNo from ${packet.toString('hex')}: ${err.toString()}`);
                }
            }
    ...
    deserialize(buffer) {
        try {
            ...
            if (error instanceof RangeError) {
                ...
                throw new TypeError(`[GenericReplyMsg] Unable to read msgNo from ${buffer.toString('hex')}: ${String(error)}`); // skipcq: JS-0378
            }
        }
    ...
    deserialize(buffer) {
        try {
            ...
            if (error instanceof RangeError) {
                ...
                throw new TypeError(`[GenericRequestMsg] Unable to read msgNo from ${buffer.toString('hex')}: ${String(error)}` // skipcq: JS-0378
                );
            }
        }
    ...
    serialize() {
        try {
            ...
            if (error instanceof Error) {
                throw new TypeError(`[NPSMsg] Error in serialize(): ${error.toString()}`);
            }
...
export class NPSPersonaMapsMessage extends NPSMessage {
    ...
    dumpPacket() {
        ...
        message = message.concat(`personaCount:        ${this.personaCount.toString()}`);
        for (const persona of this.personas) {
            message = message.concat(`
          maxPersonaCount:     ${this.deserializeInt8(persona.maxPersonas).toString()}
          id:                  ${this.deserializeInt32(persona.id).toString()}
          shardId:             ${this.deserializeInt32(persona.shardId).toString()}
...
export class NPSUserStatus extends NPSMessage {
    ...
    fetchPrivateKeyFromFile(privateKeyPath) {
        try {
            ...
            if (error instanceof Error) {
                throw new TypeError(`[npsUserStatus] Error loading private key: ${error.message.toString()}`);
            }
    ...
    extractSessionKeyFromPacket(packet) {
        ...
        const privateKey = this.fetchPrivateKeyFromFile(APP_CONFIG.MCOS.CERTIFICATE.PRIVATE_KEY_FILE);
...
export class NPSUserInfo extends NPSMessage {
    ...
    dumpInfo() {
        ...
        message = message.concat(`UserId:        ${userIdString}
         UserName:      ${userNameString}
         UserData:      ${userDataStringHex}
         [/NPSUserInfo]======================================`); // skipcq: JS-0378
        return message;
    }
}
/**
