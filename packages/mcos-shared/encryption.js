...
export function cipherBufferDES (encryptionSession, data) {
  if (encryptionSession.cipher) {
    const ciphered = encryptionSession.cipher.update(data)
    return {
...
export function decipherBufferDES (encryptionSession, data) {
  if (encryptionSession.decipher) {
    const deciphered = encryptionSession.decipher.update(data)
    return {
      session: encryptionSession,
      data: deciphered
    }
   ...
   */
   export function decryptBuffer (dataConnection,  buffer) {
       ...
       const deciphered = encryptionSession.decipher.update(buffer)
       return {
        session: encryptionSession,
        data: deciphered
      }
