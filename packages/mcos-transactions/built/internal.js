...
import { decryptBuffer, errorMessage } from 'mcos-shared'
...
function tryDecryptBuffer (message, dataConnection) {
  try {
    ...
    return {
      err: new Error(`Decrypt() exception thrown! Disconnecting...conId:${dataConnection.connectionId}: ${errorMessage(error)}`),
      data: null
    }
  }
}
