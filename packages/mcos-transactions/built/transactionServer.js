...
export class MCOTServer {
  ...
  async clientConnect (connection, packet) {
    ...
    const result = await this.databaseManager.fetchSessionKeyByCustomerId(customerId)
  ...
  decryptBuffer (message, newConnection) {
    ...
    log.debug(`Full packet before decrypting: ${encryptedBuffer.toString('hex')}`)
    log.debug(`Message buffer before decrypting: ${encryptedBuffer.toString('hex')}`)
  ...
  async messageReceived (message, con) {
    ...
    if (this.isEncryptedFlagSet(message, newConnection)) {
      if (!newConnection.isSetupComplete) {
        return {
          err: new Error(`Decrypt() not yet setup! Disconnecting...conId: ${con.id}`),
          data: null
        }
      }
  ...
  tryDecryptBuffer (message, newConnection) {
    try {
      ...
      return {
        err: new Error(`Decrypt() exception thrown! Disconnecting...conId:${newConnection.id}: ${errorMessage(error)}`),
        data: null
      }
    }
  }
