 ...
 * @param {Error} options.error The error
 * @param {import("pino").Logger} [options.log=getServerLogger({ module: "socketErrorHandler" })] The logger to use
 */
...
export function onSocketConnection({
    ...
    const portOnDataHandler = getOnDataHandler(
        fetchStateFromDatabase(),
        localPort,
    );
    ...
    incomingSocket.on("data", (/** @type {Buffer} */ incomingDataAsBuffer) => {
        ...
        portOnDataHandler({
            ...
            .then(
                (
                    /** @type {import("../../shared/State.js").ServiceResponse} */ response,
                ) => {
