...
export async function npsPortRouter({
	...
	socket.on("data", async (data) => {
		try {
			...
			const packetCount = data.toString("hex").split(separator.toString("hex"))
				.length - 1;
...
function parseInitialMessage(data: Buffer): BytableMessage {
	try {
		...
		getServerLogger(
			"gateway.npsPortRouter/parseInitialMessage",
			"gateway",
		).debug(`Parsing initial message: ${data.toString("hex")}`);
		...
		getServerLogger(
			"gateway.npsPortRouter/parseInitialMessage",
			"gateway",
		).debug(`Parsed initial message: ${message.serialize().toString("hex")}`);
		return message;
	} catch (error) {
	...
	switch (connectionPort) {
		...
		case 9001: {
			...
			if (roomServer === undefined) {
				log.warn(
					{ id: connectionId, port: connectionPort },
					"No room server found for port",
				);
				break;
			}
			log.debug(
