...
const createTaggedSocket = (
    socketOverrides: Partial<any> = {},
    id = 'test-id',
) => ({
    rawSocket: createMockSocket(socketOverrides) as any,
    connectionId: id,
...
describe('mcotsPortRouter', () => {
    ...
    it('should log an error and close the socket if local port is undefined', async () => {
        const taggedSocket: TaggedSocket = createTaggedSocket({
            localPort: undefined,
        });
        await mcotsPortRouter({ taggedSocket, log: mockLogger });
        expect(taggedSocket.rawSocket.end).toHaveBeenCalled();
    ...
    it('should handle data event and route initial message', async () => {
        const on = vi.fn((event, callback) => {
            if (event === 'data') {
                callback(
                    Buffer.from([
                        0x74, 0x65, 0x73, 0x74, 0x2d, 0x64, 0x61, 0x74, 0x61,
                    ]),
                );
            }
        });
    ...
    it('should log socket end event', async () => {
        ...
        const taggedSocket: TaggedSocket = createTaggedSocket({ on });
        await mcotsPortRouter({ taggedSocket, log: mockLogger });
