...
function npsSocketWriteIfOpen (conn, buffer) {
  ...
  if (sock.writable) {
    ...
    throw new Error(`Error writing ${buffer.toString('hex')} to ${sock.remoteAddress} , ${String(sock)}`)
  }
  return conn
}
/**
...
export class LobbyServer {
  ...
  async dataHandler (rawPacket) {
    ...
    log.debug(`Received Lobby packet: ${JSON.stringify({ localPort, remoteAddress })}`)
  ...
  async _npsRequestGameConnectServer (connection, rawData) {
    ...
    const keys = await databaseManager
      ...
      .catch((/** @type {unknown} */ error) => {
        if (error instanceof Error) {
          log.debug(`Unable to fetch session key for customerId ${customerId.toString()}: ${error.message})}`)
        }
        log.error(`Unable to fetch session key for customerId ${customerId.toString()}: unknown error}`)
        return undefined
      })
    if (keys === undefined) {
