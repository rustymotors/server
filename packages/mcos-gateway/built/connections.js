...
export function selectConnection(socket) {
    ...
    if (typeof existingConnection !== 'undefined') {
        log.info(`I have seen connections from ${socket.remoteAddress} on ${socket.localPort} before`);
    ...
    log.info(`I have not seen connections from ${socket.remoteAddress} on ${socket.localPort} before, adding it.`);
    connectionList.push(newConnection);
    log.debug(`Connection with id of ${newConnection.id} has been added. The connection list now contains ${connectionList.length} connections.`);
...
export function updateConnectionByAddressAndPort(address, port, newConnection) {
    ...
    try {
        const index = connectionList.findIndex((c) => c.socket.remoteAddress === address && c.socket.localPort === port);
        connectionList.splice(index, 1);
        /** @type {import('mcos-shared/types').SocketWithConnectionInfo} */
...
function findConnectionByAddressAndPort(remoteAddress, localPort) {
    const record = connectionList.find((c) => {
        const match = remoteAddress === c.socket.remoteAddress &&
...
export function findOrNewConnection(socket) {
    ...
    const existingConnection = findConnectionByAddressAndPort(socket.remoteAddress, socket.localPort);
    ...
    log.debug(`Connection with id of ${newConnection.modern.id} has been added. The connection list now contains ${updatedConnectionList.length} connections.`);
    return newConnection;
}
/**
    ...
    switch (localPort) {
        ...
        case 7003: {
            ...
            ${JSON.stringify({
                ...
                return {
                    err: new Error(`Error in the recieved packet: ${errorMessage(error)}`),
                    data: null
                };
            }
            try {
                ...
                log.error(errorMessage(error));
                return {
                    err: new Error(`There was an error processing the data: ${errorMessage(error)}`),
                    data: null
                };
            }
        }
