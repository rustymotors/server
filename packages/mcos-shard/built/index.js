...
export class ShardServer {
    ...
    _generateShardList() {
        ...
        const shardClockTower = new ShardEntry('The Clocktower', 'The Clocktower', 44, shardHost, 8226, shardHost, 7003, shardHost, 0, '', 'Group-1', 88, 2, shardHost, 80);
        this._possibleShards.push(shardClockTower.formatForShardList());
        const shardTwinPinesMall = new ShardEntry('Twin Pines Mall', 'Twin Pines Mall', 88, shardHost, 8226, shardHost, 7003, shardHost, 0, '', 'Group-1', 88, 2, shardHost, 80);
...
`;
    ...
    handleRequest(request, response) {
        ...
        if (request.url === '/ShardList/') {
            log.debug(`Request from ${request.socket.remoteAddress} for ${request.method} ${request.url}.`);
        ...
        log.info(`Unknown Request from ${request.socket.remoteAddress} for ${request.method} ${request.url}`);
        return response;
    }
    /**
