...
export async function handleSelectGamePersona(requestPacket) {
    ...
    log.debug(`[npsSelectGamePersona] responsePacket's data prior to sending: ${responsePacket.getPacketAsString()}`);
...
export class PersonaServer {
    ...
    async logoutGameUser(data) {
      ...
      ${JSON.stringify({
        ...
        responsePacket.dumpPacket();
        log.debug(`[npsLogoutGameUser] responsePacket's data prior to sending: ${responsePacket.getPacketAsString()}`);
        return Promise.resolve(responsePacket);
    }
    /**
    ...
    async validateLicencePlate(data) {
      ...
      ${JSON.stringify({
        ...
        responsePacket.dumpPacket();
        log.debug(`[npsCheckToken] responsePacket's data prior to sending: ${responsePacket.getPacketAsString()}`);
        return Promise.resolve(responsePacket);
    }
    /**
    ...
    async validatePersonaName(data) {
      ...
      ${JSON.stringify({
        ...
        responsePacket.dumpPacket();
        log.debug(`[npsValidatePersonaName] responsePacket's data prior to sending: ${responsePacket.getPacketAsString()}`);
        return Promise.resolve(responsePacket);
    }
    /**
     *
    ...
    async getPersonasByCustomerId(customerId) {
        const results = personaRecords.filter((persona) => persona.customerId === customerId);
    ...
    async getPersonaMaps(data) {
      ...
      ${JSON.stringify({
        ...
        log.debug(`${personas.length} personas found for ${customerId.readUInt32BE(0)}`);
