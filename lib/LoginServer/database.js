...
async function createDB() {
  ...
  const status = await db.run(
    'CREATE TABLE IF NOT EXISTS sessions (customer_id INTEGER NOT NULL UNIQUE, session_key TEXT, s_key TEXT, context_id TEXT, connection_id INTEGER)',
    [],
  );
  return status;
}
...
async function updateSessionKey(customerId, sessionKey, contextId, connectionId) {
  ...
  const db = await sqlite.open(databasePath);
  const status = await db.run(
    'INSERT OR REPLACE INTO sessions (customer_id, session_key, s_key, context_id, connection_id) VALUES ($1, $2, $3, $4, $5)',
...
module.exports = {
  createDB, fetchSessionKeyByConnectionId, updateSessionKey,
};
