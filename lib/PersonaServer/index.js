 ...
 * @param {Buffer} data
 */
async function npsLogoutGameUser(socket, data) {
 ...
 * @param {Buffer} rawData
 */
async function personaDataHandler(con, rawData) {
  const requestCode = rawData.readUInt16BE().toString(16);

  if (requestCode === '503') {
    await npsSelectGamePersona(con.sock, rawData);
    return con;
  }

  if (requestCode === '50f') {
    await npsLogoutGameUser(con.sock, rawData);
    return con;
  }

  if (requestCode === '532') {
    await npsGetPersonaMaps(con.sock, rawData);
    return con;
  }
