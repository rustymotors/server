...
function personaDataHandler(socket, rawData) {
    ...
    default:
        util.dumpRequest(socket, rawData)
        throw new Error(
            `Unknown code ${requestCode} was recieved on port 8228`
        )
    }
}
...
function personaListener(socket) {
    ...
    socket.on('data', data => {
        personaDataHandler(socket, data)
    })
    socket.on('error', err => {
...
PersonaServer.prototype.start = function start(callback) {
    net
        .createServer(socket => {
            personaListener(socket)
        })
        .listen('8228', '0.0.0.0', () => {
